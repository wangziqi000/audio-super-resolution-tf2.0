...
Altered from: https://github.com/kuleshov/audio-super-res
Original author: Volodymyr Kuleshov et al.
Ziqi: generate signal with two downsampling scales(e.g. 1kHz and 4KHz)
"""
...
parser.add_argument('--scale_low', type=int, default=2,
  help='scaling factor')
parser.add_argument('--scale_high', type=int, default=2,
  help='scaling factor')
...
def add_data(h5_file, inputfiles, args, save_examples=False):
  ...
  if args.interpolate:
    d, d_lr = int(args.dimension), int(args.dimension)
    s, s_lr = int(args.stride), int(args.stride)
  else:
    d, d_lr = int(args.dimension / args.scale_high), int(args.dimension / args.scale_low)
    s, s_lr = int(args.stride / args.scale_high), int(args.stride / args.scale_low)
  ...
  for j, file_path in enumerate(file_list):
    ...
    x_len = len(x)
    x = x[ : x_len - (x_len % args.scale_low)]
    ...
    if args.low_pass:
      ...
      x_lr = decimate(x, args.scale_low)
    else:
      x_lr = np.array(x[0::args.scale_low])
    
    if args.interpolate:
      x_lr = upsample(x_lr, args.scale_low)
      assert len(x) % args.scale_low == 0
      assert len(x_lr) == len(x)
    else:
      assert len(x) % args.scale_low == 0
      assert len(x_lr) == len(x) / args.scale_low

    x = x_preserve
    x_len = len(x)
    x = x[ : x_len - (x_len % args.scale_high)]
    ...
    if args.low_pass:
      ...
      # x_lr = np.array(x[0::args.scale])
      #x_lr = decimate(x, args.scale, zero_phase=True)
      x_hr = decimate(x, args.scale_high)
    else:
      x_hr = np.array(x[0::args.scale_high])
    
    if args.interpolate:
      x_hr = upsample(x_lr, args.scale_high)
      assert len(x) % args.scale_high == 0
      assert len(x_hr) == len(x)
    else:
      assert len(x) % args.scale_high == 0
      assert len(x_hr) == len(x) / args.scale_high


    # generate patches
    max_i = int(len(x_hr) - d + 1)
    for i in range(0, max_i, s):
      ...
      if args.interpolate:
        ...
        i_lr = int(i * args.scale_high / args.scale_low)
  ...
  if save_examples:
    librosa.output.write_wav('example-hr.wav', hr_patches[40], int(fs / args.scale_high), norm=False)
    librosa.output.write_wav('example-lr.wav', lr_patches[40], int(fs / args.scale_low), norm=False)
    print(hr_patches[40].shape)
    print(lr_patches[40].shape)
    print(hr_patches[40][:10])
    print(lr_patches[40][:10])
