...
class Model(object):
  ...
  def create_objective(self, X, Y, opt_params):
    ...
    snr = 20 * tf.compat.v1.log(sqrn_l2_norm / sqrt_l2_loss + 1e-8) / tf.math.log(10.)
  ...
  def fit(self, X_train, Y_train, X_val, Y_val, n_epoch=100):
    ...
    if tf.compat.v1.gfile.Exists(self.logdir): tf.compat.v1.gfile.DeleteRecursively(self.logdir)
    tf.compat.v1.gfile.MakeDirs(self.logdir)
  ...
  def load_batch(self, batch, alpha=1, train=True):
    ...
    g = tf.compat.v1.get_default_graph()
    k_tensors = [n for n in g.as_graph_def().node if 'keras_learning_phase/input' in n.name]
